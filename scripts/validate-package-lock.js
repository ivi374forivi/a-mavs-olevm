...
function validateTopLevel(packageJson, packageLock) {
  ...
  if (packageLock.name !== packageJson.name) {
    error(`Package name mismatch: package.json has "${packageJson.name}", package-lock.json has "${packageLock.name}"`);
  } else {
  ...
  if (packageLock.version !== packageJson.version) {
    error(`Package version mismatch: package.json has "${packageJson.version}", package-lock.json has "${packageLock.version}"`);
  } else {
  ...
  if (!packageLock.lockfileVersion) {
    ...
    warning(`Unexpected lockfileVersion: ${packageLock.lockfileVersion} (expected 3)`);
  } else {
...
function validateRootPackage(packageJson, packageLock) {
  ...
  if (rootPackage.name !== packageJson.name) {
    error(`Root package name mismatch: expected "${packageJson.name}", got "${rootPackage.name}"`);
  } else {
    success(`Root package name matches: ${rootPackage.name}`);
  }
  
  if (rootPackage.version !== packageJson.version) {
    error(`Root package version mismatch: expected "${packageJson.version}", got "${rootPackage.version}"`);
  } else {
  ...
  success(`Validated ${Object.keys(lockDevDeps).length} devDependencies and ${Object.keys(lockDeps).length} dependencies`);
}
...
function validatePackageEntries(packageLock) {
  ...
  if (packagesWithoutIntegrity > 0) {
    warning(`${packagesWithoutIntegrity} packages are missing integrity hashes`);
  } else {
  ...
  for (const [name, paths] of packageNames.entries()) {
    if (paths.length > 1) {
      ...
      if (uniqueVersions.size < versions.length) {
        warning(`Duplicate package "${name}" with same version found at: ${paths.join(', ')}`);
        duplicateCount++;
      }
    }
  }
  ...
  for (const [key, pkg] of Object.entries(packages)) {
    ...
    for (const [depName, depVersion] of Object.entries(deps)) {
      ...
      const found = Object.keys(packages).some(pkgKey => {
        return pkgKey.includes(`node_modules/${depName}`) || 
               (pkgKey === '' && packages[pkgKey].name === depName);
      });
      
      if (!found) {
        error(`Package "${key}" declares dependency "${depName}" which is not found in the lock file`);
        missingDependencies++;
      }
    }
  }
...
function validatePackageLock() {
  ...
  console.log(colors.blue + '  Package Lock Validation - Top to Bottom Check' + colors.reset);
